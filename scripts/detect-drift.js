...
Required tasks:
1. Open ${docsUrl} in a browser, confirm what the live /docs (or equivalent) UI shows right now, and save a baseline screenshot (production reflects pre-PR state until merge/deploy).
...
4. After your edits, run the app locally (e.g. npm install if needed, then npm run dev) or use a preview deployment URL if you have one, open /docs on that environment, and save a second screenshot of the updated docs UI.
5. Open a new pull request with all doc-related fixes and minimal supporting code changes only where required for accuracy. **Both screenshots must appear on that PR for reviewers:** embed them in the PR description (e.g. markdown images) and/or attach them via GitHub’s PR UI so they are visible on the PR itself—not only in chat or external links. Label them clearly (e.g. “Baseline: production /docs” and “After: local or preview /docs”).
...
If everything already matches, still open a PR that states that; embed the baseline screenshot in the PR description and, if you verified locally, embed a local /docs screenshot there too.`;
}
